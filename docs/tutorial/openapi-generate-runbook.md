...
| 種類 | 役割 | 出力先 |
|---|---|---|
| template runbook | 1 件の case を受け取って API を 1 回呼ぶ薄い runbook | `runbooks/generated/<tag>/<method>_<operationId>.template.yml` |
| case JSON | リクエスト値と期待値を書くファイル | `cases/generated/<tag>/<method>_<operationId>/default.json` |
| suite runbook | case JSON を読み込み、template runbook を実行する runbook | `runbooks/generated/<tag>/<method>_<operationId>.suite.yml` |

最初に編集するのは、基本的に case JSON です。template runbook と suite runbook は、まずは「case を実行するための仕組み」として読みます。

## generated と evidence の考え方

`runnora generate` の生成物は、再生成されることを前提にしたファイルです。

そのため、`runbooks/generated/` と `cases/generated/` は、OpenAPI 定義に追従するための作業場所として扱います。生成された runbook を手で大きく直して業務テストとして育てたい場合は、`runbooks/evidence/` など別の場所へコピーしてから編集します。
...
`generate` が作る template runbook は、`vars.case` に入ってきた値を使って API を呼びます。suite runbook は case JSON を読み込み、template runbook に `case` として渡します。
...
| `expect.bodyMode` | body の検証方法 |
| `expect.body` | 期待するレスポンス body |
| `expect.ignorePaths` | 検証から除外する JSON path |

最初は `expect.status` の検証だけで十分です。慣れてきたら `expect.body` を追加して、レスポンスの中身も確認します。
...
レスポンスの形まで確認したくなったら、`expect.body` に期待する一部の値を追加します。
...
`bodyMode: subset` は、レスポンス全体が完全一致しなくても、指定した一部が含まれていればよい、という考え方です。ID や日時のように毎回変わる値がある API では、最初から完全一致を狙わない方が安定します。
...
生成された suite runbook は、case JSON を読み込んで template runbook を呼び出します。
...
```text
cases/generated/pet/get_findPetsByStatus/
  available.json
  pending.json
  sold.json
```
...
`--clean` は `runbooks/generated/` と `cases/generated/` を掃除してから生成します。`runbooks/evidence/` は対象外です。
